...
// --- Env validation at startup — hard crash if missing ---
...
const NUM_OUTPUTS = parseInt(process.env.NUM_OUTPUTS ?? "1");
...
server.registerTool(
  "facetime_mcp",
  {
    title: "Generate Image via Replicate",
    description: `Send a prompt to the Replicate model and receive back a clean image URL.
...
Returns:
  ...
  async ({ prompt }) => {
    ...
    if (prediction.status === "failed") {
      return {
        content: [
          { type: "text", text: `Prediction failed: ${prediction.error}` },
        ],
      };
    }

    // Prefer: wait timed out on Replicate's end — poll until done
      ...
      if (polled.status === "failed") {
        return {
          content: [
            {
              type: "text",
              text: `Prediction failed during polling: ${polled.error}`,
            },
          ],
        };
      }
